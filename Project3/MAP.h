    ...
    void showAll()
    {
        ...
        for (size_t i = 0; i < mapCapacity; i++)
        {
            if (mapKeys[i] != nullptr)
            {
                ...
                {
                    cout << "mapKey: " << i << ": "
                         << "  key: " << curListNodeTemp->key << "  value: " << curListNodeTemp->value->passengersN
                         << "," << curListNodeTemp->value->seatsN << "," << curListNodeTemp->value->flightsN
                         << "," << curListNodeTemp->value->ori_populationN << "," << curListNodeTemp->value->des_populationN <<endl;
